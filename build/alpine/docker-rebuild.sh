...
OLD_REBUILDABLE=$(docker images ${DOCKER_USER}/alpine-rebuildable:local --quiet)
OLD_INTERMEDIATE=$(docker images ${DOCKER_USER}/alpine-intermediate:local --quiet)
...
if docker exec update-rebuilder sh -c "make fast"; then
    ...
    docker rmi ${OLD_INTERMEDIATE} || echo "WARN old intermediate image ${OLD_INTERMEDIATE} could not be removed"
    docker rmi ${OLD_REBUILDABLE} || echo "WARN old rebuildable image ${OLD_REBUILDABLE} could not be removed"
else
    echo "rebuild failed"
    docker stop update-rebuilder
fi
